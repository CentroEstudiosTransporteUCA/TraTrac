...
inverse of `current_transform`), but any *offline* consumer — notably
`scripts/render_violations.py`, which draws a `validate_trj.py` violations CSV onto
the video — has no access to that transform. The per-frame transforms exist only in
memory inside `OrbEgoMotionEstimator` during a run.
...
- `domain/stabilization.py` — `FrameTransform(frame_index, transform)`, the pure
  ...
  `Frame.index` — it matches `round(timestamp_s * fps)`, the key
  `render_violations` already derives for a violation row, so the two align on the
  same integer.
...
- `scripts/render_violations.py` — `--transforms-csv` loads the table and maps each
  violation centroid through `inverse(transform[frame])` before drawing. The
  inverse-apply is **reimplemented in plain arithmetic** (mirroring
  `Transform2D.inverse().apply()`) to keep the script standalone (stdlib + cv2).

**Config (zero-defaults).** `export.transform_csv` is a required toggleable key
(`""` = off), like `export.video_out` / `run.timing_csv`. Because it only makes
