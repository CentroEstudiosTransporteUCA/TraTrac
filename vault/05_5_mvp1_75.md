...
```bash
# Auto-derive Scale from drone metadata
uv run tratrac VIDEO --out OUT --drone-model mavic_3 --altitude-source srt

# Manual override when telemetry is unavailable
uv run tratrac VIDEO --out OUT --meters-per-pixel 0.05
```
...
- Already supports a `scale` constructor argument. MVP1.75 supplies the
  computed `meters_per_pixel` instead of the MVP1 default `1.0`.
