...
> **Note (MVP1.75+):** the Scale 1.0 pixels-as-metres mode above is the
> *library* default, not the CLI default. Since MVP1.75 the `tratrac` CLI
> requires explicit calibration (`--meters-per-pixel`, or `--drone-model` +
> `--altitude`/`.SRT`) and errors out otherwise — it will not silently emit
> the meaningless metric values described here. Construct the estimator/exporter
> directly if you deliberately want pixel-space output.
