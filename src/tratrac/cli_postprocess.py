...
def postprocess(
	...
	anchors: Annotated[
		...
		typer.Option(
			...
			exists=True,
			dir_okay=False,
			help="The run's anchor manifest.json (from --anchors-dir); maps zones authored on an "
			"anchor into the global frame. Omit for a static run (identity poses).",
		),
	] = None,
	...
	exclusion_note = f", dropped {dropped} excluded tracks" if exclusion_zones is not None else ""
	typer.echo(
		f"Post-processed {tracks} -> {out} "
		f"({len(states_by_frame)} frames, pos_noise={pos_noise}px, jerk={jerk}{exclusion_note})."
	)
