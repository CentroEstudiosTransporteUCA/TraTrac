...
def plot_smoothing(base: Trj, smooth: Trj | None, stem: str, out: Path, accel_bound: float) -> None:
	...
	for v in longest:
		a = base.by_vehicle[v]
		ax.plot(a[:, 1], a[:, 2], color="#d62728", alpha=0.35, lw=0.8)
		if smooth is not None and v in smooth.by_vehicle:
			s = smooth.by_vehicle[v]
			ax.plot(s[:, 1], s[:, 2], color="#1f77b4", lw=1.2)
	ax.set(
		title="Trajectories: baseline (red) vs smoothed (blue)", xlabel="x (px)", ylabel="y (px)"
	)
...
def plot_tracking(base: Trj, scores: np.ndarray | None, stem: str, out: Path) -> None:
	veh = base.by_vehicle
	...
	if births.size:
		ax.scatter(births[:, 0], births[:, 1], s=12, c="#2ca02c", label="birth")
		ax.scatter(deaths[:, 0], deaths[:, 1], s=12, c="#d62728", label="death")
...
def main() -> int:
	...
	for trj_path in baselines:
		...
		plot_smoothing(base, smooth, stem, out_dir, args.accel_bound)
		plot_tracking(base, scores, stem, out_dir)
		smooth_note = " (+smooth)" if smooth else ""
		print(f"{stem}: {len(base.by_vehicle)} tracks{smooth_note} -> {out_dir}/{stem}_*.png")
	return 0
