...
- `src/tratrac/` — the implementation, organized in onion layers:
  ...
  - `application/` — `TrajectoryPipeline` (perception-only per-frame orchestrator that emits a progress event stream and records each frame's tracked detections to a `TrackSink` — its primary output; when ego-motion is enabled it applies the transform to detections before tracking — MVP1.9), `track_smoothing.py` (`smooth_to_states`/`build_state` — reconstruct `VehicleState`s with kinematics from raw track samples; the offline `tratrac-postprocess` core, vault/22), `stabilization.py` (`apply_transform` — the pure detection→stabilized-frame coordinate map, see `vault/05_75_mvp1_9.md`), `NullProgressReporter` (silent default), and `config.py` (the `RunConfig` value object + `RunConfig.resolve` merge/validate, `DetectorChoice`, `ConfigError`; the pure zero-defaults run spec, see `vault/19_config_file.md`).
  ...
  - `cli_postprocess.py` / `cli_render.py` — sibling Typer apps for the other installed scripts: `tratrac-postprocess` (offline: **filter** exclusion zones track-aware, then Kalman/RTS de-jitter → smoothed `.trj`; `--exclusion-zones`/`--anchors`/`--exclusion-min-fraction`, see `vault/21`+`vault/22`), and `tratrac-render` (post-hoc overlay video ± violation marks: draws a `.trj`'s trajectories over its source clip via `OverlayVideoExporter`, reading the `.trj` with `read_trj`; see `vault/20_video_export.md`). Installed scripts: `tratrac`, `tratrac-postprocess`, `tratrac-render`.
