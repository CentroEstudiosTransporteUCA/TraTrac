...
Per `docs/TECH_STACK.md`: PyTorch runtime; NVDEC+PyAV decoding; SuperPoint+LightGlue stabilization; **RT-DETR** detection (deliberately *not* YOLO — aerial robustness over speed); **SAM2** segmentation (deliberately *not* Mask R-CNN); **BoT-SORT** tracking (deliberately *not* plain SORT); FastReID; EKF motion; OpenCV multi-homography; Parquet + FiftyOne + Docker/CUDA for MVP7. `docs/TECH_STACK.md` explains *why* each was chosen and what was rejected — preserve that reasoning when picking libraries.
